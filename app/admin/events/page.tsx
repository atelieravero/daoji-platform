...
const mockEvents = [
  {
    id: 'evt_1',
    titleEn: '7-Day Silent Zen Retreat',
    titleZh: '七日禪修營',
  ...
  {
    id: 'evt_2',
    titleEn: 'Weekly Wednesday Wisdom',
    titleZh: '週三智慧分享',
    startDate: 'Recurring',
    endDate: '',
    status: 'published',
  ...
  {
    id: 'evt_3',
    titleEn: 'Winter Calligraphy Workshop',
    titleZh: '冬季書法工作坊',
    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 p-8 h-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage your upcoming and past retreat programs.</p>
        </div>
        <a href="/admin/editor/new?type=event" className="inline-flex items-center px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm transition-colors focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none">
                ...
                <tr key={event.id} className="hover:bg-gray-50/80 transition-colors group">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="text-sm font-semibold text-gray-900">{event.titleEn}</div>
                        <div className="text-sm text-gray-500 mt-0.5">{event.titleZh}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button 
                      className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2 ${
                        event.status === 'published' ? 'bg-indigo-600' : 'bg-gray-200'
                      }`}
                      role="switch"
                      aria-checked={event.status === 'published'}
                    >
                      <span 
                        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          event.status === 'published' ? 'translate-x-4' : 'translate-x-0'
                        }`} 
                      />
                    </button>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <a href={`/admin/editor/${event.id}`} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors" title="Edit Content">
                      ...
                      <button className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Delete Event">
