...
// 🛡️ DATA PRIVACY ENFORCEMENT: Role check to filter test vs real data
export async function getFormAndSubmissions(formId: string) {
  ...
  // 2. Fetch User's Roles via Admin DB
  ...
  if (profileError || !profile || profile.status !== 'active') {
    throw new Error('Unauthorized or inactive account');
  }
