...
const AVAILABLE_ROLES: { id: UserRole, label: string }[] = [
  ...
  { id: 'form_data_viewer', label: 'Form Data Viewer' }
];
...
export default function TeamPage() {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInviteForm({ email: '', displayName: '', roles: ['form_data_viewer'] });
      loadTeam(); 
    } catch (err: any) {
  ...
  const handleRoleToggle = async (memberId: string, roleToToggle: UserRole, currentRoles: UserRole[]) => {
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredMembers.map((member) => {
                  const memberRoles = (member.roles || []) as UserRole[];
                      ...
                      <td className="px-6 py-4 relative">
                        ...
                        {isSelf ? (
                          <div className="text-xs text-gray-500 italic max-w-xs truncate">
                            {memberRoles.filter(r => r !== 'super_admin').map(r => AVAILABLE_ROLES.find(ar => ar.id === r)?.label).join(', ') || (isSuperAdmin ? '' : 'No Roles Assigned')}
                          </div>
                        ) : (
