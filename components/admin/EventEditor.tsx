...
import { Plus, Pencil, Settings2, RotateCw, CalendarDays, MapPin, Radio, Globe2, FileSignature } from 'lucide-react';
...
import FormPickerModal from '@/components/admin/events/FormPickerModal';
...
export default function EventEditor({ initialEvent, isNew = false }: EventEditorProps) {
  ...
  const [isLivestreamLive, setIsLivestreamLive] = useState<boolean>(initialEvent?.is_livestream_live ?? false);

  // Livestream Multi-Platform State
  const [streamMode, setStreamMode] = useState<'auto' | 'manual'>(
    (initialEvent?.livestream_config?.mode as 'auto' | 'manual') || 'auto'
  );
  ...
  const [linkedFormTitle, setLinkedFormTitle] = useState<string>('');
  ...
  const [isStandalone, setIsStandalone] = useState<boolean>(initialEvent?.is_standalone || false);
  ...
  const handleSave = async () => {
    ...
    const payload: Partial<EventRecord> = {
      ...
      is_livestream_live: isLivestreamLive,
  ...
  return (
    <>
        ...
        inspector={
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${isRecurring ? 'translate-x-4' : 'translate-x-0'}`} />
                    </button>
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl border border-gray-200 bg-white space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${isInPerson ? 'translate-x-4' : 'translate-x-0'}`} />
                    </button>
                  </div>
                ...
                {/* Livestream with Mode Selection */}
                <div className="p-4 rounded-xl border border-gray-200 bg-white space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                    ...
                    >
                      <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out ${isLivestream ? 'translate-x-4' : 'translate-x-0'}`} />
                    </button>
                  </div>
                  ...
                  {isLivestream && (
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl space-y-2">
                        <label className="text-xs font-bold text-gray-800 block">Activation Mode</label>
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          ...
                          >
                            ...
                            <span className="block text-[10px] text-gray-400 font-normal mt-0.5">Opens X min before session</span>
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            ...
                            <span className="block text-[10px] text-gray-400 font-normal mt-0.5">Admin triggers Go Live</span>
                          </button>
                        </div>
                      </div>

                      {/* Manual Quick Toggle when streamMode is manual */}
                      {streamMode === 'manual' && (
                        <FormToggle
                          label="Broadcast Is Active Right Now"
                          helperText="Overrides schedule to show the glowing live button to all visitors."
                          checked={isLivestreamLive}
                          onChange={setIsLivestreamLive}
                        />
                      )}
                ...
                {registrationMode === 'internal_form' && (
                  <div className="space-y-3 p-4 bg-amber-50/50 border border-amber-200 rounded-xl">
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-sm font-semibold text-gray-950">Linked Form</label>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => setShowFormPicker(true)}
                          className="flex-1 px-3 py-2 bg-white text-left text-xs font-semibold text-gray-800 border border-gray-300 rounded-lg hover:border-indigo-500 transition-colors cursor-pointer flex items-center justify-between"
                        >
                          <span className="truncate">
                            {linkedFormTitle || linkedFormId || '-- Select a form from database --'}
                          </span>
                          <FileSignature className="w-4 h-4 text-gray-400 shrink-0 ml-2" />
                        </button>
                        {linkedFormId && (
                          <button
                            type="button"
                            onClick={() => { setLinkedFormId(''); setLinkedFormTitle(''); }}
                            className="px-2.5 py-2 text-xs text-red-600 hover:bg-red-50 rounded-lg border border-gray-200 cursor-pointer"
                          >
                            Clear
                          </button>
                        )}
                      </div>
                      <p className="text-[11px] text-gray-500">Public registration button inherits status directly from this form.</p>
                    </div>
                  </div>
                )}
              ...
              {/* 6. URL SLUG, FEATURED & STANDALONE */}
              <div className="space-y-4">
                ...
                <FormToggle
                  ...
                  helperText="Hides top navigation bar and footer on public page. Opens in new tab from the calendar."
      ...
      <FormPickerModal
        ...
        selectedFormId={linkedFormId}
        onSelectForm={(form) => {
          setLinkedFormId(form.id);
          setLinkedFormTitle(form.title);
          if (form.events?.code && !code) {
            setCode(form.events.code);
          }
        }}
      />
    </>
