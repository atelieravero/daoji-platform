...
4.  **Audit Logs Table:** `audit_logs` stores Change Data Capture (CDC) mutation records via PostgreSQL triggers for `forms` and `team_members`: `id` (UUID), `created_at`, `actor_id` (UUID), `actor_name` (TEXT), `actor_email` (TEXT), `table_name` (`forms` | `team_members`), `operation` (`CREATE` | `UPDATE` | `DELETE`), `record_id`, `record_label` (human-readable title/profile snapshot), `old_values` (JSONB), and `new_values` (JSONB). Protected by default-deny RLS.
5.  **Database Indexing & Version Control:** Core logic (tables, sequence generation functions, CDC triggers, composite and GIN indexes) is persisted directly in raw `.sql` files within the repository (`supabase/migrations/`) to act as the source of truth and ensure cloud portability.
...
2.  **Action-Based Access Control (ABAC) & DRY Guards:** Admin authorization, hierarchical role assignment, and role UI labels are centrally governed by a strict TypeScript Master Matrix (`lib/permissions.ts`) acting as the application's single source of truth. Application code must never check a user's role directly; it must only verify if the user's role array grants authorization for a specific action key (e.g., `hasPermission(roles, 'forms:delete')`).
    *   **Security Layers:** All server actions use direct named exports (`export async function ...`) with inline `await requirePermission('...')` guards to maintain clean session cookie boundaries. Server Components use `requirePermission` for top-level redirects.
...
4.  **Supabase Client/Server Separation:** Use the standard user server client (`lib/supabase/server.ts`) for data mutations (`forms`, `team_members`) to preserve the request's JWT session headers for PostgreSQL `auth.uid()` trigger context. Reserve the Service Role admin client strictly for Supabase Auth admin tasks (invites, user creation) and cross-RLS aggregations (e.g., submission count joins, reading `audit_logs`).
